drop function if exists public.authoritative_upsert_dialogue_memory(text, text, uuid, uuid, text, text, text, smallint, extensions.vector, jsonb);
...
begin
  ...
  if p_embedding is not null then
    v_embedding := p_embedding::text::extensions.vector;
  end if;
...
begin
  ...
  v_embedding := p_query_embedding::text::extensions.vector;
